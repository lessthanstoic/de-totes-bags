"""
A collection of utility functions for working with data stored in AWS S3 and handling Parquet files.

This module provides functions to interact with an AWS S3 bucket and manipulate Parquet files. It utilizes the 'pandas' library to read Parquet data into DataFrames and the 'boto3' library for communication with AWS S3.
...
Functions:
    ...
    - readParquetFromBytesObject(file):
        Reads a Parquet file from a bytes-like object and returns it as a DataFrame.
...
These functions provide convenient ways to interact with S3 storage and process Parquet files, making it easier to work with data in the AWS cloud environment. They can be used in various data processing and analysis tasks within Python applications.

Note:
    - The functions in this module require the 'pandas' and 'boto3' libraries to be installed.
    - The AWS credentials and permissions associated with the application running this code must have the necessary access to the specified S3 bucket.
"""
import pandas as pd
import boto3
import io
...
def getDataFrameFromS3Parquet(bucket_name, file_name):
    ...
    Parameters:
        ...
        file_name (str): The name of the Parquet file in the S3 bucket.
...
def getFileFromS3(bucket_name, file_name):
    ...
    Returns:
        tuple: A tuple containing the file contents (bytes) and the HTTP status code of the response.
    """
...
def has_lambda_been_called():
    ...
    This function uses an environment variable called 'called_before' to keep track of whether
    the Lambda function has been called previously. If it's the first time being called, it
    sets the environment variable to 'True' and returns False. On subsequent calls, it returns True.

    Returns:
        bool: True if the Lambda function has been called before, False otherwise.
    """
    # Check if the 'called_before' environment variable exists and its value
    if os.environ.get('called_before', 'False') == 'False':
        # If not called before, set the environment variable to 'True' and return False
