...
def dim_staff_data_frame(staff_table, department_table):  
    ...
    try:
        ...
        staff_file = s3.get_object(Bucket='ingested-data-vox-indicium', Key=staff_name)
        ...
        if e.response['Error']['Code'] == 'NoSuchKey':
            raise ValueError(f"The file {staff_table} or {department_table} does not exist")
        else:
            raise e
        ...
        raise FileNotFoundError(f"The file {staff_table} or {department_table} does not exist")
