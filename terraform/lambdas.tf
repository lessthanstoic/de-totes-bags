...
resource "aws_lambda_function" "s3_file_reader" {
  ...
  depends_on = [ aws_cloudwatch_log_group.ingestion_lambda_log, aws_iam_policy.s3_write_policy ]
}
...
resource "aws_lambda_function" "data_transform" {
  ...
  runtime = var.pythonversion
  depends_on = [ aws_cloudwatch_log_group.transform_lambda_log, aws_iam_policy.s3_trans_write_policy ]
}
